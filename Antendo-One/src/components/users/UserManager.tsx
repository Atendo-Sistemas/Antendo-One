...
export const UserManager: React.FC = () => {
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    if (isDemo) {
      ...
      const simulatedUser = {
        id: `demo-simulation-user-${Date.now()}`,
        tenantId: tenant?.id || 'tenant-demo-public',
        name: name || 'Usuário simulado',
        email: email || 'usuario.simulado@demo.example',
        phone: phone || '(11) 4000-0000',
        role,
        status: 'ATIVO',
        accountType: 'TEST',
        readOnly: true,
      ...
      setUsers(current => [...current, simulatedUser]);
      setIsInviteOpen(false);
      setName(''); setEmail(''); setPhone('');
      setFeedbackMessage({ type: 'success', text: 'Simulação de usuário criada localmente. Nada foi salvo no sistema.' });
      window.setTimeout(() => setFeedbackMessage(null), 4200);
      return;
    }
    try {
  ...
  const handleSaveUser = async (e: React.FormEvent) => {
    ...
    if (isDemo) {
      setUsers(current => current.map(item => item.id === editingUser.id ? { ...item, name: editName, email: editEmail, phone: editPhone, role: editRole, status: editStatus, updatedAt: new Date().toISOString() } : item));
      setIsEditOpen(false);
      setEditingUser(null);
      setFeedbackMessage({ type: 'success', text: 'Simulação de edição aplicada localmente. Nada foi salvo no sistema.' });
      window.setTimeout(() => setFeedbackMessage(null), 4200);
      return;
    }
  ...
  const handleStartSupport = async (target: User) => {
    if (isDemo || user?.role !== 'SUPER_ADMIN' || target.role === 'SUPER_ADMIN') return;
    const confirmed = window.confirm(`Iniciar acesso de suporte para ${target.name}? Você entrará no painel desse usuário sem alterar a senha dele. A sessão expira em 30 minutos e será registrada na auditoria.`);
    if (!confirmed) return;
    try {
      setStartingSupportFor(target.id);
      await startSupportSession(target.id);
    } catch (err: any) {
      alert(err.message || 'Não foi possível iniciar o acesso de suporte.');
    } finally {
  ...
  const handleDeleteUser = async (id: string, userName: string) => {
    if (isDemo) {
      ...
      setFeedbackMessage({ type: 'success', text: 'Simulação descartada localmente. Nada foi apagado do sistema.' });
      window.setTimeout(() => setFeedbackMessage(null), 4200);
      return;
    }
                  ...
                  <tr key={u.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors">
                    ...
                    <td className="py-3.5 px-4 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        {isDemo ? (
                          <>
                            <span className="mr-1 text-[10px] font-semibold text-indigo-600 dark:text-indigo-300">Simulação</span>
                            <button onClick={() => handleOpenEdit(u)} className="rounded-lg bg-indigo-50 px-2 py-1.5 text-xs font-bold text-indigo-700 hover:bg-indigo-100 dark:bg-indigo-950/50 dark:text-indigo-300" title="Editar somente na simulação"><Edit2 className="h-3.5 w-3.5" /></button>
                            <button onClick={() => handleDeleteUser(u.id, u.name)} className="rounded-lg bg-slate-100 px-2 py-1.5 text-xs font-bold text-slate-600 hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-300" title="Descartar simulação"><Trash2 className="h-3.5 w-3.5" /></button>
                          </>
                        ) : (
                          <>
                            <button
                            ...
                            {u.role !== 'SUPER_ADMIN' && (
                              <button
                                ...
                                className="px-2.5 py-1.5 rounded-lg bg-amber-50 hover:bg-amber-100 dark:bg-amber-950/50 dark:hover:bg-amber-900/60 text-amber-700 dark:text-amber-300 transition-colors cursor-pointer inline-flex items-center gap-1 font-bold text-xs disabled:opacity-50"
                                title="Entrar no painel deste usuário em sessão temporária de suporte"
                              >
                                {startingSupportFor === u.id ? <div className="w-3.5 h-3.5 border-2 border-amber-600 border-t-transparent rounded-full animate-spin" /> : <LifeBuoy className="w-3.5 h-3.5" />}
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl max-w-lg w-full shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col max-h-[90vh]">
            <div className="px-6 py-4 bg-slate-900 text-white flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-base font-bold text-white">{isDemo ? 'Simular edição de usuário' : 'Editar Perfil do Usuário'}</h2>
                  <p className="text-xs text-slate-400">{isDemo ? 'Alteração temporária: nada será salvo no sistema' : 'Atualize dados cadastrais e permissões'}</p>
                </div>
              </div>
              <button 
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  >
                    ...
                    <option value="MOTORISTA">Motorista</option>
                    {!isDemo && <option value="SUPER_ADMIN">Super Administrador SaaS</option>}
                  </select>
                </div>
                ...
                >
                  {isSaving ? (
                    ...
                    <>
                      ...
                      <span>{isDemo ? 'Aplicar simulação' : 'Salvar Alterações'}</span>
                    </>
                  )}
                </button>
              </div>
