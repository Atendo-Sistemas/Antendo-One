...
assert.match(api, /return tenant;/, 'editor deve permitir personalização por empresa');
...
assert.match(navbar, /company-notification-templates.*Mensagens/, 'menu da empresa não aponta para o editor');
assert.match(navbar, /companyNavGroups/, 'atalho de mensagens da empresa ausente');
